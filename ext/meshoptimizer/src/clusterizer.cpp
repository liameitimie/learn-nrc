...
static bool appendMeshlet(meshopt_Meshlet& meshlet, bool force_finish, unsigned int a, unsigned int b, unsigned int c, unsigned char* used, meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, size_t meshlet_offset, size_t max_vertices, size_t max_triangles)
{
	...
	// force_finish use to finish this meshlet to avoid a triangle too far from meshlet, where work worse in virtual mesh - by Liameitimie

	if (force_finish || meshlet.vertex_count + used_extra > max_vertices || meshlet.triangle_count >= max_triangles)
	{
...
size_t meshopt_buildMeshlets(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride, size_t max_vertices, size_t max_triangles, float cone_weight)
{
	...
	// float meshlet_aabb[2][3] = { {FLT_MAX, FLT_MAX, FLT_MAX}, {-FLT_MAX, -FLT_MAX, -FLT_MAX} };
	// float meshlet_aabb_extent2 = 0;
	...
	for (;;)
	{
		...
		if (best_triangle == ~0u)
		{
			...
			// if (best_triangle != ~0u && meshlet.triangle_count > 0) {
				// compare min distance of triangle between meshlet and meshlet aabb extent
				// float min_distance2 = FLT_MAX;

				// for (int i = 0; i < 3; i++) {

				// 	int vertex_idx = indices[best_triangle * 3 + i];
				// 	const float* p = vertex_positions + vertex_positions_stride / sizeof(float) * vertex_idx;

				// 	float distance2 = 0;
				// 	for (int j = 0; j < 3; j++) {
				// 		distance2 += (p[j] - position[j]) * (p[j] - position[j]);
				// 	}
				// 	min_distance2 = fmin(min_distance2, distance2);
				// }
				
				// if (min_distance2 > meshlet_aabb_extent2) {
				// 	force_finish = true;
				// }
			// }
		}
		...
		if (appendMeshlet(meshlet, force_finish, a, b, c, used, meshlets, meshlet_vertices, meshlet_triangles, meshlet_offset, max_vertices, max_triangles))
		{
			meshlet_offset++;
			memset(&meshlet_cone_acc, 0, sizeof(meshlet_cone_acc));

			// for (int i = 0; i < 3; i++) {
			// 	meshlet_aabb[0][i] = FLT_MAX;
			// 	meshlet_aabb[1][i] = -FLT_MAX;
			// }
			// meshlet_aabb_extent2 = 0;
		}
		...
		// 	for (int j = 0; j < 3; j++) {
		// 		meshlet_aabb[0][j] = fmin(meshlet_aabb[0][j], p[j]);
		// 		meshlet_aabb[1][j] = fmax(meshlet_aabb[1][j], p[j]);
		// 	}
		// }
		// meshlet_aabb_extent2 = 0;
		// for (int i = 0; i < 3; i++) {
		// 	meshlet_aabb_extent2 += (meshlet_aabb[1][i] - meshlet_aabb[0][i]) * (meshlet_aabb[1][i] - meshlet_aabb[0][i]);
		// }
...
size_t meshopt_buildMeshletsScan(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, const unsigned int* indices, size_t index_count, size_t vertex_count, size_t max_vertices, size_t max_triangles)
{
	...
	for (size_t i = 0; i < index_count; i += 3)
	{
		...
		meshlet_offset += appendMeshlet(meshlet, false, a, b, c, used, meshlets, meshlet_vertices, meshlet_triangles, meshlet_offset, max_vertices, max_triangles);
	}
