...
void dump_code(const hlsl::CodegenResult &str, const vstd::MD5 &md5) {
    auto md5_str = md5.to_string();
    auto dump_file_name = vstd::string("hlsl_output_raster_") + md5_str + ".hlsl";
    auto f = fopen(dump_file_name.c_str(), "ab");
...
ID3D12CommandSignature* RasterShader::command_signature() const {
    // std::lock_guard lck(cmdSigMtx);
    if (_command_signature) return _command_signature.Get();
    ...
    D3D12_COMMAND_SIGNATURE_DESC cmdsig_desc = {
        ...
        .NumArgumentDescs = 1,
        .pArgumentDescs = &indirect_arg,
    };

    ThrowIfFailed(device->device->CreateCommandSignature(&cmdsig_desc, nullptr, IID_PPV_ARGS(&_command_signature)));
    return _command_signature.Get();
}
...
RasterShader *RasterShader::CompileRaster(
    ...
    auto CompileNewCompute = [&](bool writeCache) -> RasterShader * {
        ...
        if (RasterShaderDetail::PRINT_CODE) {
            RasterShaderDetail::dump_code(str, md5);
        }
