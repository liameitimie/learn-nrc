...
static size_t AddHeader(CallOpSet const &ops, luisa::BinaryIO const *internalDataPath, vstd::StringBuilder &builder, bool isRaster) {
    ...
    if (ops.test(CallOp::INDIRECT_SET_DISPATCH_KERNEL) || ops.test(CallOp::INDIRECT_SET_DISPATCH_COUNT) || ops.test(CallOp::INDIRECT_SET_DRAW_ARGUMENT)) {
...
void CodegenUtility::GetVariableName(Variable::Tag type, uint id, vstd::StringBuilder &str) {
    switch (type) {
        ...
        case Variable::Tag::OBJECT_ID:
            ...
            str << "obj_id"sv;
            break;
        case Variable::Tag::VERTEX_ID:
            LUISA_ASSERT(opt->isRaster, "vertex id only allowed in raster shader");
            str << "vt.vid"sv;
            break;
        case Variable::Tag::INSTANCE_ID:
            LUISA_ASSERT(opt->isRaster, "instance id only allowed in raster shader");
            str << "vt.iid"sv;
            break;
...
void CodegenUtility::GetFunctionName(CallExpr const *expr, vstd::StringBuilder &str, StringStateVisitor &vis) {
    ...
    switch (expr->op()) {
        ...
        case CallOp::INDIRECT_SET_DRAW_ARGUMENT: {
            str << "__set_draw_argument"sv;
        } break;
