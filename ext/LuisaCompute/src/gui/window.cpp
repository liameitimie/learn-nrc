...
float2 Window::get_cursor_pos() const noexcept {
    double2 p;
    glfwGetCursorPos(static_cast<detail::WindowImpl *>(_impl.get())->window, &p.x, &p.y);
    return float2(p.x, p.y);
}

void Window::set_cursor_disabled() noexcept {
    glfwSetInputMode(static_cast<detail::WindowImpl *>(_impl.get())->window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
}
void Window::set_cursor_normal() noexcept {
    glfwSetInputMode(static_cast<detail::WindowImpl *>(_impl.get())->window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
}
