...
void Expr<IndirectDispatchBuffer>::set_kernel(
    Expr<uint> offset,
    Expr<uint3> block_size,
    Expr<uint3> dispatch_size, Expr<uint> kernel_id
) const noexcept {
    detail::FunctionBuilder::current()->call(
...
void Expr<IndirectDispatchBuffer>::set_draw_argument(Expr<uint4> draw_arg) const noexcept {
    detail::FunctionBuilder::current()->call(
        CallOp::INDIRECT_SET_DRAW_ARGUMENT,
        {_expression, draw_arg.expression()}
    );
}
