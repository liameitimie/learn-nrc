...
public:
    DrawRasterSceneCommand(
        ...
        uint64_t indirect_buffer = 0
    ) noexcept :
        ShaderDispatchCommandBase{shader_handle, std::move(argument_buffer), argument_count},
        _rtv_texs{rtv_textures},
        _rtv_count{rtv_count},
        _dsv_tex{dsv_texture},
        _scene{std::move(scene)},
        _viewport{viewport},
        _raster_state{raster_state},
        _indirect_buffer{indirect_buffer}
    {}
...
public:
    ...
    uint64_t indirect_buffer() const noexcept { return _indirect_buffer; }
