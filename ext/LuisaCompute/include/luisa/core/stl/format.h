...
template<size_t N>
struct formatter<luisa::DMatrix<N>> {
    constexpr auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
        return ctx.end();
    }
    template<typename FormatContext>
    auto format(const luisa::DMatrix<N> &m, FormatContext &ctx) -> decltype(ctx.out()) {
        if constexpr (N == 2u) {
            return fmt::format_to(
                ctx.out(),
                FMT_STRING("double2x2("
                           "cols[0] = ({}, {}), "
                           "cols[1] = ({}, {}))"),
                m[0].x, m[0].y,
                m[1].x, m[1].y);
...
template<size_t N>
[[nodiscard]] auto to_string(DMatrix<N> m) noexcept {
    return luisa::format(FMT_STRING("({})"), m);
}
