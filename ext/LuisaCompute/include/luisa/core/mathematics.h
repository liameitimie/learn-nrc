...
template<size_t N>
[[nodiscard]] constexpr double length(Vector<double, N> u) noexcept {
    return sqrt(dot(u, u));
}
...
[[nodiscard]] constexpr auto cross(float2 u, float2 v) noexcept {
    return u.x * v.y - v.x * u.y;
}

[[nodiscard]] constexpr auto cross(double2 u, double2 v) noexcept {
    return u.x * v.y - v.x * u.y;
}
...
[[nodiscard]] constexpr bool inverse(const float2x2 m, float2x2 &inv) noexcept {
    const float det = m[0][0] * m[1][1] - m[1][0] * m[0][1];
    if (abs(det) < 1e-8) return false;

    const float inv_det = 1.0f / det;
    inv = make_float2x2(m[1][1] * inv_det,
                        -m[0][1] * inv_det,
                        -m[1][0] * inv_det,
                        +m[0][0] * inv_det);
    return true;
...
[[nodiscard]] constexpr bool inverse(const double2x2 m, double2x2 &inv) noexcept {
    const double det = m[0][0] * m[1][1] - m[1][0] * m[0][1];
    if (abs(det) < 1e-12) return false;

    const double inv_det = 1.0 / det;
    inv = make_double2x2(m[1][1] * inv_det,
                        -m[0][1] * inv_det,
                        -m[1][0] * inv_det,
                        +m[0][0] * inv_det);
    return true;
...
[[nodiscard]] constexpr bool inverse(const float3x3 m, float3x3 &inv) noexcept {// from GLM
    const float det = m[0].x * (m[1].y * m[2].z - m[2].y * m[1].z)
    ...
    const float inv_det = 1.0f / det;
    inv = make_float3x3(
        (m[1].y * m[2].z - m[2].y * m[1].z) * inv_det,
        (m[2].y * m[0].z - m[0].y * m[2].z) * inv_det,
        (m[0].y * m[1].z - m[1].y * m[0].z) * inv_det,
        (m[2].x * m[1].z - m[1].x * m[2].z) * inv_det,
        (m[0].x * m[2].z - m[2].x * m[0].z) * inv_det,
        (m[1].x * m[0].z - m[0].x * m[1].z) * inv_det,
        (m[1].x * m[2].y - m[2].x * m[1].y) * inv_det,
        (m[2].x * m[0].y - m[0].x * m[2].y) * inv_det,
        (m[0].x * m[1].y - m[1].x * m[0].y) * inv_det
    );
    return true;
}

[[nodiscard]] constexpr bool inverse(const double3x3 m, double3x3 &inv) noexcept {// from GLM
    const double det = m[0].x * (m[1].y * m[2].z - m[2].y * m[1].z)
                        - m[1].x * (m[0].y * m[2].z - m[2].y * m[0].z)
                        + m[2].x * (m[0].y * m[1].z - m[1].y * m[0].z);
    if (abs(det) < 1e-12) return false;
...
[[nodiscard]] constexpr bool inverse(const float4x4 m, float4x4 &inv) noexcept {// from GLM
    ...
    if (abs(dot1) < 1e-8) return false;

    const auto one_over_determinant = 1.0f / dot1;
    inv = float4x4{inv_0 * one_over_determinant,
...
[[nodiscard]] constexpr bool inverse(const double4x4 m, double4x4 &inv) noexcept {// from GLM
    const auto coef00 = m[2].z * m[3].w - m[3].z * m[2].w;
    const auto coef02 = m[1].z * m[3].w - m[3].z * m[1].w;
    const auto coef03 = m[1].z * m[2].w - m[2].z * m[1].w;
    const auto coef04 = m[2].y * m[3].w - m[3].y * m[2].w;
    ...
    const auto dot1 = dot0.x + dot0.y + dot0.z + dot0.w;
    if (abs(dot1) < 1e-12) return false;

    const auto one_over_determinant = 1.0f / dot1;
    inv = double4x4{inv_0 * one_over_determinant,
                    inv_1 * one_over_determinant,
                    inv_2 * one_over_determinant,
                    inv_3 * one_over_determinant};
