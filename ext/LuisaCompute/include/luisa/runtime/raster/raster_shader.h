...
public:
    ...
    template<typename... Rtv>
    [[nodiscard]] auto draw_indirect(const IndirectDispatchBuffer &indirect_buffer, Viewport viewport, const RasterState &raster_state, DepthBuffer const *dsv, Rtv const &...rtv) && noexcept {
        if (dsv) {
            _command.set_dsv_tex(ShaderDispatchCommandBase::Argument::Texture{dsv->handle(), 0});
        } else {
            _command.set_dsv_tex(ShaderDispatchCommandBase::Argument::Texture{invalid_resource_handle, 0});
        ...
        _command.set_indirect_buffer(indirect_buffer.handle());
        _command.set_viewport(viewport);
        _command.set_raster_state(raster_state);
        return std::move(_command).build();
    }
...
//                  dsv_format,
         ...
         _uniform_size{
            ...
            ShaderDispatchCmdEncoder::compute_uniform_size(vert.arguments().subspan(1))
            + ShaderDispatchCmdEncoder::compute_uniform_size(pixel.arguments().subspan(1))
        }
#ifndef NDEBUG
